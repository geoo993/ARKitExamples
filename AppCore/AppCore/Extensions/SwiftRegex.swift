...
public class SwiftRegex: NSObject { //, ExpressibleByBooleanLiteral {
...
public func ~= ( left: SwiftRegex, right: [String] ) -> Bool {
    ...
    return left.substituteMatches( substitution: {
        ...
        if matchNumber == right.count {
            // FIXME: Error 'stop is a 'let' constant: Figure out how to write this in swift-3
//            stop.memory = true
        }
...
public class RegexFile {
    ...
    deinit {
        _ = SwiftRegex.saveFile( path: filepath, newContents: contents )
    }
